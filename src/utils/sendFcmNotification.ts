...
export async function sendFcmNotification(
  ...
  try {
    ...
    console.log(`📦 Complete data payload:`, JSON.stringify(notificationData, null, 2));

    // Determine notification type based on data
    const isCalling = notificationData.isCalling === "true";
    const isBooking = notificationData.type === "booking_request" || 
                     Boolean(notificationData.bookingId && notificationData.pickupLocation);
    const shouldShowAsCard = isCalling || isBooking;

    console.log(`🎯 Notification type: ${shouldShowAsCard ? 'OVERLAY CARD' : 'REGULAR NOTIFICATION'}`);
    ...
    const message: admin.messaging.Message = {
      ...
      android: {
        ...
        notification: shouldShowAsCard ? undefined : {
          title: title,
          body: body,
      ...
      apns: {
        ...
        payload: {
          aps: {
            ...
            alert: shouldShowAsCard ? undefined : {
              title: title,
              body: body
            },
            sound: "default",
            ...
            "content-available": 1,
            "mutable-content": 1,
            category: shouldShowAsCard ? "BOOKING_CALL_CATEGORY" : "GENERAL_CATEGORY"
          },
          // Custom data for iOS
    ...
    console.log(`🎯 Expected behavior: ${shouldShowAsCard ? 'Show as overlay card with timer + progress bars' : 'Show in notification bar'}`);
...
export async function sendTestBookingNotification(fcmToken: string): Promise<string> {
...
export async function sendTestCallNotification(fcmToken: string): Promise<string> {
...
export async function sendTestRegularNotification(fcmToken: string): Promise<string> {
...
export async function sendTaxiSureCallNotification(
  ...
  const title = options.userType === 'emergency' ? 'Emergency Call 🚨' : 'Incoming Call 📞';
...
export async function sendTaxiSureRegularNotification(
  ...
  title: string,
  body: string,
  notificationType: 'general' | 'promotion' | 'booking_confirmed' | 'payment_success' | 'ride_started' | 'driver_arrived' | 'rating_request',
  additionalData?: Record<string, string>
): Promise<string> {
...
export function validateFcmToken(token: string): boolean {
  ...
  const hasValidFormat = token.includes(':') && (token.includes('APA91b') || token.includes('AAAA'));
  
  if (!isValidLength) {
    console.error("❌ FCM token length is invalid (should be 100-200 characters)");
  }
  
  if (!hasValidFormat) {
    console.error("❌ FCM token format is invalid (should contain ':' and 'APA91b' or 'AAAA')");
  }
